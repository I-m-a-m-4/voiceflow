...
export function BillingModal({ isOpen, onClose }: BillingModalProps) {
  ...
  const handleUpgrade = (plan: any) => {
    ...
    const config = {
      ...
      customizations: {
        ...
        logo: 'https://voiceflow.space/icon.png',
      },
    };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[700px] bg-[#111111] text-white border-gray-800 p-0 overflow-hidden">
        <div className="p-8">
          ...
          <div className="flex justify-end mb-6">
            <div className="bg-[#1C1C1E] p-1 rounded-full flex items-center gap-2 border border-gray-800">
              <span className={`px-4 py-1.5 text-sm rounded-full cursor-pointer transition-colors ${!isAnnual ? 'bg-[#333333] text-white' : 'text-gray-400'}`} onClick={() => setIsAnnual(false)}>
                Monthly
              </span>
              <span className={`px-4 py-1.5 text-sm rounded-full cursor-pointer transition-colors flex items-center gap-2 ${isAnnual ? 'bg-white text-black' : 'text-gray-400'}`} onClick={() => setIsAnnual(true)}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            ...
            {/* Pro + Undetectability */}
            <div className="bg-gradient-to-b from-zinc-800 to-zinc-900 rounded-2xl p-6 text-white flex flex-col h-full relative border border-orange-500/30">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-medium text-lg text-orange-400">Pro + Undetectability</h3>
                <span className="bg-orange-500/20 text-orange-400 px-2 py-1 rounded-md text-xs font-medium border border-orange-500/30">Popular</span>
              </div>
              <div className="flex items-baseline gap-2 mb-6">
                <span className="text-white/60 line-through text-xl">${plans[1].monthlyPrice}</span>
                <span className="text-4xl font-bold text-white">${plans[1].discountedMonthly}</span>
                <span className="text-white/80">/month</span>
              </div>
              ...
              <ul className="space-y-4 mb-8 flex-1 relative z-10">
                <li className="flex items-start gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-white/80 leading-snug block">{plans[1].features[1]}</span>
                  </div>
                </li>
              </ul>
          ...
          <div className="border-t border-gray-800 pt-6 flex items-center justify-between text-sm">
            <div>
              <span className="text-gray-400 block mb-1">Free plan</span>
              <span className="text-2xl font-bold text-white">$0</span>
            </div>
            <div className="grid grid-cols-2 gap-x-8 gap-y-3">
              <div className="flex items-center gap-2 text-gray-300">
                <div className="bg-gray-800 p-0.5 rounded-full"><Check size={12} className="text-gray-400" /></div>
