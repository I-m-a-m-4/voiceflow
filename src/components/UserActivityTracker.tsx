...
function claimFirstSession(uid: string): boolean {
    try {
        const key = `voiceflow_seen_before_${uid}`;
...
export function UserActivityTracker() {
    ...
    useEffect(() => {
        ...
        const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
            ...
            const sessionIdKey = `voiceflow_session_id_${user.uid}`;
    ...
    useEffect(() => {
        ...
        const checkAndUpdateActivity = async (user: any) => {
            ...
            try {
                ...
                if (!lastUpdate || now - parseInt(lastUpdate) > 5 * 60 * 1000) {
                    ...
                    let country = sessionStorage.getItem('voiceflow_session_country') || '';
                    if (country === 'Unknown') country = '';
                    if (!country) {
                        country = await getCountryFromIP();
                        if (country && country !== 'Unknown') {
                            sessionStorage.setItem('voiceflow_session_country', country);
                        }
                    }
        ...
        const flushOnUnload = () => {
            ...
            const routeLog = sessionLogRef.current.slice(-400);
            if (routeLog.length > 0) {
                const sid = sessionStorage.getItem(`voiceflow_session_id_${uid}`);
                if (sid) {
