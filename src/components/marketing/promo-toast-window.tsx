...
export function PromoToastWindow() {
  ...
  const evaluateVisibility = useCallback((currentConfig: PromoToastConfig) => {
    ...
    if (typeof window !== 'undefined') {
      const storageKey = `voiceflow_promo_dismissed_${currentConfig.id}`;
  ...
  useEffect(() => {
    ...
    window.addEventListener('voiceflow_trigger_promo_test', handleTestTrigger);
    return () => window.removeEventListener('voiceflow_trigger_promo_test', handleTestTrigger);
  }, []);
  ...
  const handleDismiss = (e?: React.MouseEvent) => {
    ...
    if (typeof window !== 'undefined' && config.id) {
      const storageKey = `voiceflow_promo_dismissed_${config.id}`;
              ...
              <div className="p-4 pt-3.5 space-y-2.5">
                ...
                <div>
                  <h3 className="font-extrabold text-sm sm:text-base leading-snug text-foreground tracking-tight">
                    {config.title || 'Special Voiceflow Offer'}
                  </h3>
                  {config.description && (
