...
export const FirebaseProvider = ({ children, firebaseApp, firestore, auth }: FirebaseProviderProps) => {
  const [userAuthState, setUserAuthState] = useState<UserAuthState>(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        const raw = localStorage.getItem('voiceflow_auth_session');
    ...
    const unsubscribe = onAuthStateChanged(
      ...
      (firebaseUser) => { // Auth state determined
        if (firebaseUser) {
          ...
          try {
            localStorage.setItem('voiceflow_auth_session', JSON.stringify(sessionData));
          } catch {}
        } else {
          try {
            localStorage.removeItem('voiceflow_auth_session');
          } catch {}
        }
