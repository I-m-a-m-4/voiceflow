...
/**
 * @fileOverview A simple support chat bot for Voiceflow.
 *
 * - voiceflowSupportChat - A function that answers user questions.
 * - VoiceflowSupportChatInput - Input type.
 * - VoiceflowSupportChatOutput - Output type.
 */
...
const VoiceflowSupportChatInputSchema = z.object({
  query: z.string().describe('The user\'s question about the Voiceflow app.'),
  uid: z.string().optional().describe('The authenticated user\'s ID'),
});
export type VoiceflowSupportChatInput = z.infer<typeof VoiceflowSupportChatInputSchema>;

const VoiceflowSupportChatOutputSchema = z.object({
...
export type VoiceflowSupportChatOutput = z.infer<typeof VoiceflowSupportChatOutputSchema>;
...
// analysis flows do.
export async function voiceflowSupportChat(input: VoiceflowSupportChatInput, idToken?: string): Promise<VoiceflowSupportChatOutput> {
  const uid = await requireUser(idToken);
  return withUserCredits(uid, 'voiceflowSupportChat', () => supportChatFlow({ ...input, uid }));
}
...
const prompt = ai.definePrompt({
   ...
   * Voiceflow" out of the fixed feature list pasted below it. There is no arithmetic to get
  ...
  input: {schema: VoiceflowSupportChatInputSchema},
  output: {schema: VoiceflowSupportChatOutputSchema},
  system: `You are Zen AI, a highly advanced, extremely intelligent, and friendly AI strategist built specifically for the Voiceflow high-performance business suite. Voiceflow is an enterprise-ready, offline-first application that handles inventory, sales, and complex business analytics.
        
**Your Core Directives (CRITICAL GUARDRAILS):**
1.  **Scope Restriction:** You MUST answer questions ONLY related to Voiceflow's features, usage, and capabilities as outlined in the "VOICEFLOW APP FEATURES" section. If a user asks about anything unrelated (e.g., general knowledge, coding, writing essays, other software), politely decline and remind them you are dedicated to Voiceflow support.
...
6.  Keep your responses highly professional, incredibly helpful, and strictly focused on solving the user's Voiceflow-related problem.
...
const supportChatFlow = ai.defineFlow(
  {
    ...
    inputSchema: VoiceflowSupportChatInputSchema,
    outputSchema: VoiceflowSupportChatOutputSchema,
  },
  async (input: any) => {
