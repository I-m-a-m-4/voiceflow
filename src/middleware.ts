...
export function middleware(req: NextRequest) {
    ...
    // Get hostname (e.g. voiceflow.space, shop.voiceflow.space, localhost:3000)
    ...
    // Logic for voiceflow.space subdomains
    if (domain.endsWith(".voiceflow.space") && domain !== "voiceflow.space") {
        const subdomain = domain.replace(".voiceflow.space", "");

        // If it's a restricted subdomain (like www.voiceflow.space), let it pass through (don't rewrite to store)
        if (restrictedSubdomains.includes(subdomain)) {
            return NextResponse.next();
        }

        // Rewrite to the store path
        // e.g. voiceflow-shop.voiceflow.space/cart -> voiceflow.space/store/voiceflow-shop/cart
