...
export function useFCM() {
            ...
            if (newPermission === 'granted') {
                ...
                if (token) {
                    ...
                    if (user) {
                        ...
                        const tokenRef = doc(firestore, `users/${(user.id || (user as any).uid)}/fcmTokens`, token);
    ...
    const unsubscribe = async () => {
        ...
        setIsLoading(true);
        try {
            const tokenRef = doc(firestore, `users/${(user.id || (user as any).uid)}/fcmTokens`, fcmToken);
