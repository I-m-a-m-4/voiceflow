        ...
        try {
            if (adminAuth) {
                ...
                if (targetUser.email) {
                    await sendEmail({
                        to: targetUser.email,
                        name: targetUser.displayName || 'Voiceflow User',
                        subject: payload.title,
                        body: `<p style="font-size: 16px; color: #333;">${payload.body}</p><br><hr style="border-top: 1px solid #eaeaea; margin: 20px 0;"><p style="color: #666; font-size: 14px;">Reply to this email or log into your Voiceflow account to continue the conversation.</p>`,
                        from: 'Voiceflow Support <support@voiceflow.space>',
                        replyTo: 'support@voiceflow.space',
                        type: 'support'
                    });
                }
            }
