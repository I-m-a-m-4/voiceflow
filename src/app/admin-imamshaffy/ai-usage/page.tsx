...
function foldDays(docs: AiDailyStats[]): WindowTotals {
  ...
  return {
    turns,
    blocked: docs.reduce((s, d) => s + (Object.values(d.blocked ?? {}) as number[]).reduce((a, n) => a + Number(n), 0), 0),
    ...
    toolCalls: docs.reduce(
      (s, d) => s + (Object.values(d.tools ?? {}) as number[]).reduce((a, n) => a + Number(n), 0),
      0,
    ),
  };
}
...
export default function AdminAIUsage() {
  ...
  const intentChart = useMemo(() => {
    const merged = mergeCountMaps(days.map((d: any) => d.intents));
    const total = Object.values(merged).reduce((s: number, n: number) => s + n, 0);
    return INTENTS
      .map((i: any) => ({
        id: i.id,
        name: i.label,
  ...
  const keywordChart = useMemo(() => {
    const merged = mergeCountMaps(days.map((d: any) => d.keywords));
    return topEntries(merged, 22).map((e: any) => ({ name: e.key, value: e.value }));
  }, [days]);
                      ...
                      <div className="flex-1 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            ...
                            backgroundColor:
                              TOOL_GROUPS.find((g: any) => g.id === t.group)?.color ?? '#94a3b8',
                          }}
                        />
                      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
        <MetricCard
          ...
          delta={{
            ...
            windowLabel: priorWindowLabel,
            format: (v: any) => (v > 0 ? `${(v / 1000).toFixed(1)}s` : '—'),
          }}
          spark={{
            data: spark.latency,
            format: (v: any) => (v > 0 ? `${(v / 1000).toFixed(1)}s` : 'no turns'),
          }}
        ...
        <MetricCard
          ...
          delta={{
            ...
            higherIsBetter: false,
            windowLabel: priorWindowLabel,
            format: (v: any) => `${Number(v).toFixed(1)}%`,
          }}
          spark={{ data: spark.blocked, format: (v: any) => `${v} blocked` }}
        ...
        >
          ...
          {blockedChart.length > 0 && (
            <div className="mt-2 space-y-1">
              {blockedChart.map((row: any) => (
                <DetailRow key={row.key} label={row.name} value={Number(row.value).toLocaleString()} />
              ))}
            </div>
          )}
          <ReferenceNote
        ...
        <MetricCard
          ...
          spark={{ data: spark.errors, format: (v: any) => `${v} errors` }}
        ...
        <MetricCard
          ...
          spark={{ data: spark.proposals, format: (v: any) => `${v} proposals` }}
          summary={
            totals.turns > 0
          ...
          <CardContent>
            {keywordChart.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2 items-center content-start min-h-[300px]">
                {keywordChart.map((k: any, i: number) => {
            ...
            {blockedChart.length === 0 ? (
              ...
              <div className="space-y-3">
                {blockedChart.map((row) => {
                  const max = Number(blockedChart[0]?.value) || 1;
                  return (
                    <div key={row.key}>
