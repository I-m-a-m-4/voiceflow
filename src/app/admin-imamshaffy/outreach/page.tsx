...
import { getInsightCohorts, takeInsightCohort } from '@/lib/insight-cohort';
...
function EmailMarketingConsole() {
  ...
  React.useEffect(() => {
    ...
    if (!cohortId) return;
    const cohorts = getInsightCohorts(users, businesses);
    const cohort: any = takeInsightCohort(cohorts, cohortId);
    if (!cohort) {
      setAudienceNotice(
    ...
    const known = new Set(profiles.map(p => p.userId).filter(Boolean));
    const userIds: string[] = cohort.userIds || [];
    const resolved = userIds.filter(id => known.has(id));
    setSelectedIds(new Set(resolved));
    setTab('compose');

    const dropped = userIds.length - resolved.length;
    setAudienceNotice(
      `${resolved.length} selected from “${cohort.label ?? cohortId}”`
      + (dropped > 0 ? ` · ${dropped} no longer match an active account` : ''),
    );
  }, [searchParams, audienceLoading, profiles, users, businesses]);
        ...
        <TabsContent value="results" className="mt-4">
          <CampaignResults
            ...
            onFollowUp={(emails: string[]) => {
              ...
              const validUserIds = matched.map(p => p.userId).filter((id): id is string => Boolean(id));
              setSelectedIds(new Set(validUserIds));
