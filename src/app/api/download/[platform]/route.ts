...
export async function GET(
  ...
  try {
    const res = await fetch('https://api.github.com/repos/I-m-a-m-4/voiceflow-releases/releases/latest?t=' + Date.now(), {
  ...
  if (adminFirestore) {
    try {
      const visitorCookie = request.cookies.get('voiceflow_visitor_id');
  ...
  if (!downloadUrl) {
    switch (platform) {
      case 'windows':
        ...
        downloadUrl = `https://github.com/I-m-a-m-4/voiceflow-releases/releases/download/v${version}/voiceflow_${version}_x64-setup.exe`;
        break;
      case 'macos-silicon':
        downloadUrl = `https://github.com/I-m-a-m-4/voiceflow-releases/releases/download/v${version}/voiceflow_${version}_aarch64.dmg`;
        break;
      case 'macos-intel':
        downloadUrl = `https://github.com/I-m-a-m-4/voiceflow-releases/releases/download/v${version}/voiceflow_${version}_x64.dmg`;
        break;
      case 'android':
        // Match the signed APK pattern perfectly
        downloadUrl = `https://github.com/I-m-a-m-4/voiceflow-releases/releases/download/v${version}/voiceflow-v${version}-SIGNED.apk`;
        break;
      default:
