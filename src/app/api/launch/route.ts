...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.warn('[api/launch] Telemetry store warning (bypassed):', writeErr?.message || writeErr);
    }

    return NextResponse.json({ ok: true }, { headers: corsHeaders });
  } catch (err: any) {
    console.error('[api/launch] failed to process launch request:', err?.message || err);
    return NextResponse.json({ ok: true, fallback: true }, { headers: corsHeaders });
  }
}
