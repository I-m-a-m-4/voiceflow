 ...
 *
 * Followed from a browser at voiceflow.space rather than fetched cross-origin, so
...
<body style="margin:0;padding:0;background:#f4f2ef;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;">
  <div style="max-width:520px;margin:0 auto;padding:56px 20px;">
    <div style="background:#fff;border:1px solid #ececec;border-radius:16px;padding:36px 32px;text-align:center;">
      <div style="font-size:22px;font-weight:800;letter-spacing:-0.6px;color:${ORANGE};margin-bottom:26px;">voiceflow</div>
      ${bodyHtml}
    </div>
    <p style="text-align:center;font-size:12px;color:#9a3412;margin-top:20px;">
      &copy; ${new Date().getFullYear()} Voiceflow POS &amp; Inventory
    </p>
  </div>
</body>
</html>`;
...
export async function GET(req: NextRequest) {
  ...
  return page(
    ...
    `<p style="${HEADING}">Stop receiving these emails?</p>
     <p style="${TEXT}">You will still get essential account and receipt emails &mdash; this only turns off the occasional note from us about using Voiceflow.</p>
...
export async function POST(req: NextRequest) {
  ...
  const done = page(
    ...
    `<p style="${HEADING}">You are unsubscribed</p>
     <p style="${TEXT}">We will not send you any more emails about using Voiceflow. Essential account emails &mdash; receipts, password resets, security notices &mdash; still come through.</p>
