...
import { voiceflowSupportChat, type VoiceflowSupportChatInput } from '@/ai/flows/support-chat-flow';
...
const faqItems: { question: string; answer: React.ReactNode; id?: string; tags: string[] }[] = [
  // --- CATEGORY: OFFLINE & SYNC (5) ---
  {
    ...
    answer: (
        <div className="space-y-4">
            <p className="text-sm">Voiceflow's core philosophy is that **your business should never wait for the internet.**</p>
  ...
  },
  {
      ...
      answer: <p className="text-sm">Yes. Voiceflow stores your entire product and customer database locally. Searching by name, SKU, or category is just as fast (and sometimes faster) when offline because it queries your local drive directly.</p>
  },
  ...
  // --- CATEGORY: POS & SALES (8) ---
  {
    ...
    answer: (
      <div className="space-y-4">
        <p className="text-sm">Voiceflow is optimized for **Zero-Latency Scanning**. Simply connect your USB or Bluetooth scanner. In the POS interface, just scan, and the item is instantly added to the cart.</p>
  ...
  },
  {
      ...
      answer: <p className="text-sm">When adding the product to your inventory, set its unit to 'kg' or 'lbs'. When adding it to the cart in the POS, you will be prompted to enter the exact weight, and Voiceflow will calculate the price automatically.</p>
  },
  {
      question: "Does Voiceflow support thermal printers and cash drawers?",
      tags: ["printer", "thermal", "cash-drawer", "hardware"],
      answer: <p className="text-sm">Yes. Voiceflow communicates directly with your operating system's print spooler. Any ESC/POS thermal printer (58mm or 80mm) installed on your OS will work perfectly. Cash drawers connected via the printer's RJ11 port will kick open automatically on cash sales.</p>
  },
  {
  ...
  {
      question: "What are the minimum system requirements for Voiceflow Desktop?",
      tags: ["system", "requirements", "windows", "mac", "linux"],
      answer: <p className="text-sm">Voiceflow is optimized to run on modest hardware. Minimum: Windows 10/11 (64-bit), 4GB RAM, and 500MB disk space. For the best experience with Zen AI local processing, we recommend 8GB RAM and an SSD.</p>
  },
  {
      question: "How do I update the desktop application to the latest version?",
      tags: ["update", "version", "download", "auto-update"],
      answer: <p className="text-sm">Voiceflow checks for updates every time it launches. If a new version (e.g., v0.5.8) is available, you will see a "New Version Available" button in the Top Title Bar. Simply click it to download and relaunch with the latest features.</p>
  },
  {
      question: "Can I run Voiceflow on multiple computers at the same time?",
  ...
  },
  {
      ...
      answer: <p className="text-sm">This usually happens if the application is prevented from writing to its data folder. Try running Voiceflow as an Administrator, or ensure that your Antivirus isn't blocking the `voiceflow.db` file in your AppData directory.</p>
  },
  {
  ...
  },
  {
      ...
      answer: <p className="text-sm">Go to **Inventory {'->'} Import**. Download our template CSV, fill in your product names, SKUs, and prices, and upload it. Voiceflow can process thousands of products in seconds.</p>
  },
  {
    question: "Setting up Low Stock & Expiry Date alerts",
    tags: ["alerts", "inventory", "stock", "expiry"],
    answer: <p className="text-sm">Under each product's settings, you can define a **Minimum Stock Level**. Voiceflow will proactively alert you when stock falls below this. You can also enable **Expiry Date tracking** for perishable goods.</p>
  },
  {
  ...
  },
  {
      ...
      answer: <p className="text-sm">Use the **Visual Count** feature in the Inventory module. You can scan or select items and enter their actual shelf count. Voiceflow will log the difference as a 'Stock Adjustment' in your audit trails.</p>
  },
  {
      question: "Does Voiceflow support composite products (Bundles/Kits)?",
  ...
  },
  {
      ...
      answer: <p className="text-sm">Yes. Voiceflow's core intelligence module is deterministic and runs locally on your computer. It analyzes your SQLite data to provide "Customer Intelligence" and "Sales Forecasts" even without internet.</p>
  },
  {
  ...
  // --- CATEGORY: CUSTOMERS & CRM (5) ---
  {
    ...
    answer: <p className="text-sm">Voiceflow includes a powerful **CRM suite**. You can enable loyalty points and set reward thresholds (e.g., 1 point per ₦100 spent). Customers can then redeem these for discounts during checkout.</p>
  },
  {
  ...
  },
  {
      ...
      answer: <p className="text-sm">Yes. Voiceflow allows you to record an order as 'Unpaid'. It will track the outstanding balance on the customer's profile, and you can record payments against that debt later to balance the books.</p>
  },
  ...
  },
  {
      ...
      answer: <p className="text-sm">Absolutely. Voiceflow is an **Enterprise-Ready architecture**. You can create multiple 'Outlets' and monitor stock transfers, individual shop profits, and total company performance from one master dashboard.</p>
  },
  {
  ...
  {
      question: "How do I upgrade or cancel my Voiceflow subscription?",
  ...
  },
  {
      ...
      answer: <p className="text-sm">Yes. We offer custom **Voiceflow Enterprise** solutions for high-volume retailers. Please contact our Executive Support team via the chat above or email `enterprise@voiceflow.ai` for a tailored quote and dedicated account manager.</p>
  },
  {
  ...
  {
      question: "Does Voiceflow offer a free trial for the Pro features?",
...
function ZenAIChatBot({ userProfile }: { userProfile?: UserProfile }) {
    ...
    React.useEffect(() => {
        let greeting = "Hi! I'm Zen AI. Ask me anything about Voiceflow, or ask me to add a product to your inventory!";
        try {
            const historyStr = sessionStorage.getItem('voiceflow_recent_pages');
            if (historyStr) {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            const response = await voiceflowSupportChat({ query: input }, await idToken());
            let aiText = response.answer || (response as any).response || t('support.aiFallback');
            const isUnanswered = response.isUnanswered || false;

            if (isUnanswered) {
                aiText = "I'm not quite sure about that. I specialize in Voiceflow features like inventory, POS, and analytics. If you need help with something specific, please rephrase or reach out to our human support team!";
            }
