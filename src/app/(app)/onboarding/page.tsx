...
const ONBOARDING_TRANSLATIONS: Record<string, {
  ...
  en: {
    title: "Set Up Your Voiceflow Store, {name}",
    ...
    noteLanguage: "Voiceflow is set to English by default. If another language suits you better, pick it here — you can change it any time in Settings → General.",
  ...
  es: {
    title: "Configura tu tienda Voiceflow, {name}",
    ...
    noteLanguage: "Voiceflow está configurado en inglés por defecto. Si otro idioma te conviene más, selecciónalo aquí; puedes cambiarlo en cualquier momento en Ajustes → General.",
  ...
  fr: {
    title: "Configurez votre boutique Voiceflow, {name}",
    ...
    noteLanguage: "Voiceflow est configuré en anglais par défaut. Si une autre langue vous convient mieux, choisissez-la ici — vous pouvez la changer à tout moment dans Paramètres → Général.",
  ...
  de: {
    title: "Richten Sie Ihren Voiceflow-Shop ein, {name}",
    ...
    noteLanguage: "Voiceflow ist standardmäßig auf Englisch eingestellt. Wenn eine andere Sprache besser passt, wählen Sie sie hier aus — Sie können sie jederzeit unter Einstellungen → Allgemein ändern.",
  ...
  it: {
    title: "Configura il tuo negozio Voiceflow, {name}",
    ...
    noteLanguage: "Voiceflow è impostato su Inglese come predefinito. Se preferisci un'altra lingua, selezionala qui — puoi cambiarla in qualsiasi momento in Impostazioni → Generale.",
  ...
  pt: {
    title: "Configure sua loja Voiceflow, {name}",
    ...
    noteLanguage: "Voiceflow está configurado em Inglês por padrão. Se outro idioma for melhor para você, escolha-o aqui — você pode mudar a qualquer momento em Configurações → Geral.",
  ...
  ar: {
    title: "إعداد متجر Voiceflow الخاص بك، {name}",
    ...
    noteLanguage: "تم إعداد Voiceflow باللغة الإنجليزية افتراضيًا. إذا كانت هناك لغة أخرى تناسبك بشكل أفضل، فاخترها من هنا — يمكنك تغييرها في أي وقت في الإعدادات ← عام.",
  ...
  hi: {
    title: "अपना Voiceflow स्टोर सेटअप करें, {name}",
    ...
    noteLanguage: "Voiceflow डिफ़ॉल्ट रूप से अंग्रेजी पर सेट है। यदि कोई अन्य भाषा आपके लिए बेहतर है, तो उसे यहाँ चुनें — आप इसे किसी भी समय सेटिंग → सामान्य में बदल सकते हैं।",
  ...
  ja: {
    title: "Voiceflowストアを設定する、{name}さん",
    ...
    noteLanguage: "Voiceflowはデフォルトで英語に設定されています。他の言語をご希望の場合はこちらで選択してください。設定 → 一般 からいつでも変更できます。",
  ...
  ko: {
    title: "Voiceflow 상점 설정하기, {name}님",
    ...
    noteLanguage: "Voiceflow는 기본적으로 영어로 설정되어 있습니다. 다른 언어가 더 편하시다면 여기서 선택해 주세요. 설정 → 일반 에서 언제든지 변경하실 수 있습니다.",
  ...
  zh: {
    ...
    noteLanguage: "Voiceflow 默认设置为英文。如果您更偏好其他语言，请在此处选择 — 您可以随时在“设置 → 常规”中进行更改。",
    selectIndustry: "选择行业",
...
export default function OnboardingPage() {
  ...
  const onSubmit = async (data: OnboardingFormValues) => {
    ...
    try {
      ...
      sessionStorage.setItem('voiceflow_onboarding_complete', 'true');
      localStorage.setItem('voiceflow_needs_tour', 'true');

      toast({ variant: 'success', title: 'Setup Complete!', description: 'Welcome to your Voiceflow dashboard.' });
