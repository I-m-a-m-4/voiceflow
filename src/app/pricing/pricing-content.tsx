...
export default function PricingContent() {
    const { t } = useI18n();
    const { initializePayment, isLoading: isPaymentLoading } = useFlutterwave();
    ...
    const handleCheckout = useCallback((plan: 'pro' | 'biz') => {
        const amounts: Record<string, Record<string, number>> = {
            pro:  { NGN_monthly: PRO_MONTHLY_NGN,  NGN_yearly: PRO_YEARLY_NGN,  USD_monthly: PRO_MONTHLY_USD * 100,  USD_yearly: PRO_YEARLY_USD * 100  },
            biz:  { NGN_monthly: BIZ_MONTHLY_NGN,  NGN_yearly: BIZ_YEARLY_NGN,  USD_monthly: BIZ_MONTHLY_USD * 100,  USD_yearly: BIZ_YEARLY_USD * 100  },
        };
        const key = `${currency}_${billingCycle}`;
        const amount = amounts[plan][key];
        const planLabel = `${plan === 'pro' ? 'Pro' : 'Business'} ${billingCycle === 'monthly' ? 'Monthly' : 'Yearly'}`;

        // Prompt for email since user may not be logged in
        ...
        initializePayment({
            ...
            onSuccess: (response) => {
                toast({
                    ...
                    description: `Your VoiceFlow ${planLabel} subscription is now active. Ref: ${response.tx_ref}`,
                });
            },
            onClose: () => {},
        });
    }, [currency, billingCycle, initializePayment, toast]);
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div className="relative flex flex-col p-8 bg-white border-2 border-dashed border-primary rounded-lg shadow-2xl shadow-primary/10">
                                ...
                                <div className="mt-auto pt-8">
                                    ...
                                    >
                                        {isPaymentLoading ? 'Processing...' : t('pricing.ctaPro')}
                                    </Button>
                                </div>
                            </div>
                            ...
                            <div className="relative flex flex-col p-8 bg-white border-2 border-dashed border-slate-200 rounded-lg shadow-sm">
                                ...
                                <div className="mt-auto pt-8">
                                    <Button
                                        size="lg"
                                        className="w-full"
                                        onClick={() => handleCheckout('biz')}
                                        disabled={isPaymentLoading}
                                    >
                                        {isPaymentLoading ? 'Processing...' : t('pricing.ctaBiz')}
                                    </Button>
                                </div>
                            </div>
                        </div>
