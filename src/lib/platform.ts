 ...
 * `https://voiceflow.space` there sends every local `npm run dev` request to
 ...
 *
 * The `|| 'https://voiceflow.space'` fallback matters because `.env` is gitignored:
...
const FIRST_LAUNCH_KEY = 'voiceflow_first_launch_at';
...
const PLAY_STORE_URL =
  'https://play.google.com/store/apps/details?id=com.voiceflow.app';
const MS_STORE_URL =
...
const PLAY_STORE_PROTOCOL = 'market://details?id=com.voiceflow.app';
 ...
 * scheme is dropped by the CSP, which is why the Settings "Rate Voiceflow" button
