...
function drawSelectScene(ctx: CanvasRenderingContext2D, frame: number) {
  appShell(ctx, 60, 64, STAGE.w - 120, STAGE.h - 150, 'Voiceflow — Point of Sale', 1);
...
function drawCustomerScene(ctx: CanvasRenderingContext2D, frame: number) {
  appShell(ctx, 60, 64, STAGE.w - 120, STAGE.h - 150, 'Voiceflow — Customer', 1);
...
function drawPaymentScene(ctx: CanvasRenderingContext2D, frame: number) {
  appShell(ctx, 60, 64, STAGE.w - 120, STAGE.h - 150, 'Voiceflow — Payment', 1);
...
function drawDoneScene(ctx: CanvasRenderingContext2D, frame: number) {
  const shell = appShell(ctx, 60, 64, STAGE.w - 120, STAGE.h - 150, 'Voiceflow — Review', 1);
  ctx.save();
  ...
  if (rt > 0) {
    ...
    text(ctx, 'VOICEFLOW', rx + rw / 2, ry + 58, {
...
export const posDemo: Demo = {
  ...
  draw(ctx, frame, theme) {
    ...
    titleCard(
      ctx, 'One tap to sold.', 'Voiceflow Point of Sale',
