...
export function resolveNotificationLink(notif: NotificationLike): string {
  ...
  if (mentions('voiceflow on windows') || mentions('microsoft store') || mentions('windows desktop')) {
    return '/download';
  }
  if (mentions('mobile app') || mentions('android app') || mentions('google play')) {
    return 'https://play.google.com/store/apps/details?id=com.voiceflow.app';
  }
