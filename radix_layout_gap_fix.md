...
This document details the exact root cause of the Radix UI layout shift / gap bug in Voiceflow and documents the permanent, zero-bug solution for future reference.
...
3. Radix UI dynamically injects inline styles directly onto `<body>`:
   ...
   ```
4. In custom desktop application shells (e.g., Voiceflow's fixed Tauri/Next.js layout with internal scroll areas), this injected padding shifts the main layout container, causing layout shift and gap artifacts.
