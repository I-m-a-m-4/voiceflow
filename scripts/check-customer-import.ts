...
/*
 * The old dialog wrote `${name}${4 random chars}@voiceflow-import.local` into every
 * row. Re-importing an export of that data must not carry them back in.
 */
const placeholder = draftsFor(
  table(['Name', 'Email'], [['Musa Ibrahim', 'musaibrahim9x2k@voiceflow-import.local']]),
);
...
// what the file actually said next to what Voiceflow made of it.
 ...
 */
const hasPlaceholder = existing({ id: 'c8', name: 'Musa', email: 'musa9x2k@voiceflow-import.local' });
eq(
 ...
 * double-counts every purchase Voiceflow has already recorded, which inflates the
...
eq('a smaller total does not reduce what Voiceflow knows', lower.totalSpent, undefined);
