 ...
 * 230px shorter than a real 1080p window — so the bottom of every Voiceflow page sat
...
function parseArgs(argv) {
  const out = {
    ...
    music: process.env.VOICEFLOW_RECORD_MUSIC || null,
    /*
...
Microsoft Store delivery:
  ...
  A trailer is five files, not one. After the take, build the rest and check it:
    node scripts/record/store.mjs marketing-out/voiceflow-trailer-desktop-light.mp4
...
Credentials (never passed as flags, never printed):
  .env.recorder  ->  VOICEFLOW_RECORD_EMAIL=...
                     VOICEFLOW_RECORD_PASSWORD=...
`;
...
async function record(opts, flowId, device, theme, creds, live) {
  ...
  try {
    ...
    const outFile = path.join(opts.outDir, `voiceflow-${stamp}.${opts.format}`);
    log('encoding…');
...
async function main() {
  ...
  const creds = {
    email: process.env.VOICEFLOW_RECORD_EMAIL || fileEnv.VOICEFLOW_RECORD_EMAIL,
    password: process.env.VOICEFLOW_RECORD_PASSWORD || fileEnv.VOICEFLOW_RECORD_PASSWORD,
  };
  if (!creds.email || !creds.password) {
    console.error(
      '\n  Missing credentials.\n\n' +
      '  Create .env.recorder in the project root (it is gitignored):\n\n' +
      '    VOICEFLOW_RECORD_EMAIL=demo@yourbusiness.com\n' +
      '    VOICEFLOW_RECORD_PASSWORD=...\n\n' +
