...
check('placeholder email detected', isPlaceholderEmail('adeyemia1b2@voiceflow-import.local'));
check('placeholder email detected case-insensitively', isPlaceholderEmail('X@VOICEFLOW-IMPORT.LOCAL'));
...
eq('realEmail returns null for a placeholder', realEmail({ email: 'a@voiceflow-import.local' }), null);
...
{
  const a = cust({ id: 'a', email: 'x1@voiceflow-import.local' });
  const b = cust({ id: 'b', email: 'x2@voiceflow-import.local' });
  const r = computeCustomerHealth([a, b], { isMultiBranch: false })!;
...

{
  ...
  const b = cust({
    id: 'b',
    name: 'Okafor Ada',
    email: 'x@voiceflow-import.local',
    phone: '',
    code: 'VIP9',
    totalSpent: 100,
    loyaltyPoints: 5,
...

{
  ...
  const primary = cust({ id: 'p', email: 'p@voiceflow-import.local', totalSpent: 500 });
