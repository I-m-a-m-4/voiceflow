...
function processManifest(filePath) {
  ...
  if (missing.length > 0 && content.includes('<application')) {
    const block = `
    <!-- Voiceflow System-level Hardware Integrations -->
${missing.map((e) => `    ${e.tag}`).join('\n')}
`;
    content = content.replace('<application', `${block}\n    <application`);
  }

  // Inject host domain intent filter so Android Credential Manager associates voiceflow.space with the app
  if (!content.includes('android:host="voiceflow.space"') && content.includes('</activity>')) {
    const domainFilter = `
        <intent-filter android:autoVerify="true">
            <action android:name="android.intent.action.VIEW" />
            <category android:name="android.intent.category.DEFAULT" />
            <category android:name="android.intent.category.BROWSABLE" />
            <data android:scheme="https" android:host="voiceflow.space" />
        </intent-filter>
    `;
  ...
  console.log(`SUCCESS: Patched AndroidManifest.xml with hardware entries and voiceflow.space domain binding.`);
}
