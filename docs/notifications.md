...
`src/lib/notification-rules.ts` holds the rules that decide when Voiceflow interrupts
...
`localStorage.voiceflow_last_viewed_errors`, and for `added` documents under 60 seconds old
...
- **An anomaly is a condition, not an exception.** Three of the five that exist throw
  ...
  throw".
- **Throttled per code, per device, per day** via `voiceflow_anomaly_<code>` plus an
  in-session `Set`. A desktop install with a locked `voiceflow.db` hits the same anomaly on
...
- The popup title is **"Voiceflow Anomaly Detected"** rather than "New Developer Log", so
...
6. **Is it a trigger?** Check `localStorage.voiceflow_trg_sent_<businessId>` — the id may
