...
**Caching does not rescue this at Voiceflow's volume.** Implicit caching is free but needs
...
Two flows do run cheaper: `voiceflowSupportChat` and `productTroubleshoot` override the model
...
- **`FLOW_CREDITS`** is the one unavoidable hand-maintained table. The Genkit flows
  ...
  `voiceflowSupportChat` 1. If a flow is ever moved onto the AI SDK and reports usage,
...
This is why Voiceflow's own cap table has a **separate route and toolkit**:
...
AI who owns Voiceflow, what it is valued at, and what each investor paid. There is
