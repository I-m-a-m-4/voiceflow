...
Getting a shop's stock — and its customer book — into Voiceflow, from whatever they
actually have.
...
1. **SKU.** Voiceflow has no separate barcode field — the scanner matches on `sku`, so a
...
**regaining window focus is the trigger** — Voiceflow reads the clipboard by itself,
...
```
`${sanitizedName}${4 random chars}@voiceflow-import.local`
```
...
snapshot to a running total double-counts every purchase Voiceflow already recorded, and
