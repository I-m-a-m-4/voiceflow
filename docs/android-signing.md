...
How Voiceflow's Android releases get signed, and how to fix the
...
The registered upload key, alias `voiceflow`:
...
2. **Alias mismatch.** `key.properties` used `keyAlias=upload`, but the
   registered key's alias is `voiceflow`. Gradle signed with whatever the
...
- **Never commit a keystore or its password.** `voiceflow.keystore` is in this
  repo's history with `voiceflowpass` in a committed workflow. The repo is
