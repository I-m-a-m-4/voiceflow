# Voiceflow — the technology stack, end to end

What Voiceflow is built from, why each piece is there, and what the less obvious
...
Voiceflow is a **local-first retail POS/ERP** that ships as a **desktop app**
...
- **SQLite via `@tauri-apps/plugin-sql`** — the local-first engine.
  ...
  customers, receipts) into `voiceflow.db` so a shop keeps operating if the
...
- **A standing rule: entitlement fields are owner-only** — plan, access level
  ...
  client-written.
- **Android signing** — the registered upload key alias is `voiceflow`, and the
