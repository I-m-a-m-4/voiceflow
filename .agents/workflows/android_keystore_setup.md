...
const alias = 'upload';
const dname = "CN=Voiceflow App, OU=Engineering, O=Voiceflow, L=San Francisco, S=California, C=US";
...
- **CI/CD**: GitHub Actions will not find `key.properties` (since it's git-ignored). It will produce an unsigned `.aab` (or fall back to `voiceflow.keystore` / `jarsigner` logic inside the workflow YAML). This prevents `os error 28` or missing file crashes during automated deployment pipelines!
