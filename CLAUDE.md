...
throttle, so a desktop install whose `voiceflow.db` is locked or corrupt gets zero products
...
- **Celebration history is keyed per business** (`voiceflow_ach_seen_<id>`), and so are
  the goals (`voiceflow_goals_<id>`). Both were bare global keys — the same trap CLAUDE.md
...
badges unread against `localStorage.voiceflow_last_viewed_errors` and calls
...
- **Throttled per code, per device, per day** (`voiceflow_anomaly_<code>` + a session set).
  A device with a broken `voiceflow.db` hits the same anomaly on every launch; the point is
...
- The admin popup reads `data.message || data.errorMessage || …` and titles anomalies
  **"Voiceflow Anomaly Detected"**, so they are distinguishable from crash logs at a glance.
...
- Mounted **outside** `FirebaseClientProvider`: a broken Firebase config is itself a
  ...
  depend on it.
- **It only measures builds that ship with it** — the route needs a `voiceflow.space`
...
- **What actually cut it: send less in, and never let a tool forget.**
  ...
  **set-wide in one loop**, because one hook per tool is one chance per tool to
  ...
  rephrase). `voiceflowSupportChat` and `productTroubleshoot` do override the model to
...
- **The customer book has its own importer on the same rails.**
  ...
  - **Only `name` is required, and nothing is invented.** The dialog it replaces
    demanded an email and **fabricated one** — `${name}${4 random chars}@voiceflow-import.local`
  ...
  - **Updates fill blanks only, and the two running totals take the `max`, never the
    ...
    every purchase Voiceflow already recorded. An update that would write nothing is
    dropped, not queued.
...
- **Production `voiceflow.space` currently cannot be recorded in a fresh headless
...
- **The achievement modal will eventually cover a click.** Every take runs in a
  throwaway Chrome profile, so `voiceflow_ach_seen_<businessId>` is always empty and
...
- The registered upload key has alias **`voiceflow`**, SHA1
...
never restored, so `voiceflow.space/api/<name>` answers the **HTML 404 page**.
...
curl -s -o /dev/null -w "%{http_code} %{content_type}\n" https://voiceflow.space/api/<name>
```
...
**Fixing a route only takes effect once voiceflow.space is redeployed.** Rebuilding
